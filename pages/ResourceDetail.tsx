...
export default function ResourceDetail() {
    ...
    const fetchRelated = useCallback(async (cursor?: string) => {
        ...
        const items = Array.from({ length: count }).map((_, i) => ({
            ...
            type: 'RESOURCE',
            title: `Design Principle #${(cursor ? parseInt(cursor) : 0) + i + 1}: ${['Balance', 'Contrast', 'Hierarchy', 'Repetition'][i % 4] || 'Concept'}`,
            content: "Deep dive into the core concepts of visual design...",
        ...
        const injectedItems = db.injectContent(items, 'resourceFeed');
        ...
        <div className="flex flex-col min-h-screen bg-dark-850 text-[#f4f4f5] font-sans pb-32">
             ...
             <div className="sticky top-0 z-50 bg-dark-900/90 backdrop-blur-lg border-b border-white/5 h-16 flex items-center px-8">
                ...
                >
                    ...
                    <span className="text-lg font-black tracking-tighter uppercase italic">Article Reader</span>
                </button>
            </div>
            ...
            <div className="max-w-[1000px] mx-auto w-full px-6 py-12">
                ...
                <div className="text-center mb-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
                    <h1 className="text-4xl md:text-5xl font-black text-white uppercase italic tracking-tighter mb-8 leading-[1.1]">
                        {resource.title}
                    </h1>
                    
                    <div className="flex flex-wrap items-center justify-center gap-6 text-xs text-gray-400 font-medium border-t border-b border-white/5 py-6">
                        <div className="flex items-center gap-3">
                            <img src={resource.authorAvatar} className="w-10 h-10 rounded-full border border-white/10" alt={resource.author} />
                            <div className="text-left">
                                <span className="text-white font-bold uppercase tracking-wide block leading-none mb-1">{resource.author}</span>
                        ...
                        <div className="flex items-center gap-2">
                                <span className="bg-brand-500/10 text-brand-500 px-3 py-1 rounded-sm text-[9px] font-black uppercase tracking-widest border border-brand-500/20">
                                {resource.category}
                            </span>
                            {resource.tags.map(tag => (
                                <span key={tag} className="text-gray-500 text-[10px] font-bold uppercase tracking-wider hover:text-white transition-colors cursor-pointer px-2">#{tag}</span>
                            ))}
                        </div>
                        ...
                        <div className="flex items-center gap-6">
                            <span className="flex items-center gap-1.5 uppercase tracking-wide font-bold"><Clock size={16} className="text-gray-500"/> {resource.readTime}</span>
                            <span className="flex items-center gap-1.5"><Eye size={16} className="text-gray-500"/> {resource.views}</span>
                        </div>
                    </div>
                </div>

                {/* Featured Image */}
                <div className="w-full aspect-[21/9] rounded-xl overflow-hidden mb-12 border border-white/10 shadow-2xl relative group">
                ...
                <div className="mt-20 pt-12 border-t border-white/5">
                    ...
                    <form onSubmit={handlePostComment} className="mb-12 flex gap-4">
                        <div className="w-12 h-12 rounded-full bg-brand-600 flex items-center justify-center font-black text-white text-xs shrink-0 border-2 border-dark-900 shadow-lg">YOU</div>
                    ...
                    <div className="space-y-8">
                        {comments.map(comment => (
                            <div key={comment.id} className="flex gap-5 group animate-in slide-in-from-bottom-2 duration-300">
                                <img src={comment.avatar} className="w-12 h-12 rounded-full border border-white/10 shrink-0 object-cover" alt={comment.user} />
                ...
                {/* Explore More - Standardized 3-Col Grid */}
